...
// YENİ: sendEmailVerification eklendi
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const texts = { /* ... Önceki adımdaki texts objesinin tamamı ... */ };

    let currentLang = 'tr';

    // YENİ: Profesyonel Bildirim Fonksiyonu
    function showNotification(message, type = 'error') {
        notificationMessage.textContent = message;
        notificationMessage.className = `notification ${type}`; // 'success' veya 'error' class'ı ekler
        
        // 5 saniye sonra bildirimi yavaşça kaldır
        setTimeout(() => {
            notificationMessage.classList.add('hidden');
        }, 5000);
    }

    // ... updateTexts, handleSelection, switchTab fonksiyonları aynı ...

    // --- GÜNCELLENMİŞ FORM FONKSİYONLARI ---
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        signInWithEmailAndPassword(auth, email, password)
            .then((userCredential) => {
                // YENİ: E-posta onayı kontrolü
                if (userCredential.user.emailVerified) {
                    // E-posta onaylı, giriş başarılı
                    console.log("Giriş başarılı ve e-posta onaylı:", userCredential.user);
                    showNotification(texts[currentLang].login_success || "Giriş başarılı! Ana menüye yönlendiriliyorsunuz...", 'success');
                    // GELECEKTE BURASI BİZİ ANA MENÜYE GÖTÜRECEK
                } else {
                    // E-posta onaysız, girişe izin verme
                    auth.signOut(); // Kullanıcıyı güvenlik için sistemden at
                    showNotification(texts[currentLang].login_unverified || "Lütfen önce e-postanızı onaylayın.", 'error');
                }
            })
            .catch((error) => {
                console.error("Giriş hatası:", error);
                showNotification(texts[currentLang].login_fail || "Giriş başarısız oldu. Lütfen bilgilerinizi kontrol edin.", 'error');
            });
    });
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // YENİ: Kullanıcıya doğrulama e-postası gönder
            await sendEmailVerification(user);

            await setDoc(doc(db, "users", user.uid), userData);
            
            console.log("Kullanıcı veritabanına kaydedildi ve doğrulama e-postası gönderildi.");
            showNotification(texts[currentLang].register_success || "Kayıt başarılı! Lütfen e-posta kutunuzu kontrol ederek hesabınızı doğrulayın.", 'success');
    ...
    // ... Diğer tüm fonksiyonlar ve metinler aynı kalacak ...
    // Tam kodu aşağıya bırakıyorum.
});

// -------------------------------------------------------------------------------------
// YUKARIDAKİ KODUN TAM VE EKSİKSİZ HALİNİ KULLANMAN DAHA KOLAY OLACAKTIR.
// AŞAĞIDAKİNİ KOPYALA VE main.js DOSYANA YAPIŞTIR.
// -------------------------------------------------------------------------------------
...
// Senin yeni Firebase projenin kimlik bilgileri.
const firebaseConfig = {
  apiKey: "AIzaSyDAlIpvrMtKbfOJmTo1Ut4H3lV3KMePQZo",
  authDomain: "lezzet-yagmuru.firebaseapp.com",
  projectId: "lezzet-yagmuru",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showNotification(message, type = 'error') {
        ...
        setTimeout(() => { notificationMessage.className = 'notification hidden'; }, 5000);
    }
    ...
    function handleSelection(selection) {
        currentLang = selection; updateTexts(currentLang); langContainer.classList.add('fade-out');
        setTimeout(() => { langContainer.classList.add('hidden'); authScreen.classList.remove('hidden'); }, 300);
    }
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        const password = document.getElementById('register-password').value;
        const userData = {
            social: document.getElementById('register-social').value, card_gsm: document.getElementById('register-card-gsm').value,
            isFollowing: document.getElementById('follow-confirm').checked, region: currentLang, points: 0,
